# -*- coding: utf-8 -*-
"""
Created on Tue Aug 18 16:40:30 2020

@author: chmar
"""
#calculate MPI of 100 control runs
import os 
import numpy as np 
import pandas as pd 
...
# location='/scratch/cm5515/cntrl_run_60_late'
# #go there
# os.chdir(location)
# num=100
# for i in range(num):
#when does your storm start?
start=datetime(2005,8,15,1,30)

#this is from Emmanuel et al:
#%%
...
#%% find the storm center and the pressure there:
...
storm_xtimes = np.array([])
times = np.array(wrfout.variables['PSFC']).shape[0]

t=0
for t in range(times):
    # Get stuff we need for actual intensity:
    slp = np.copy(wrfout.variables['PSFC'][t,: ,:])
    land_type = np.copy(wrfout.variables['XLAND'][t, :, :])
    lats = np.copy(wrfout.variables['XLAT'][t, :, :]) 
    lons = np.copy(wrfout.variables['XLONG'][t, :, :])
    xtime = np.copy(int(wrfout.variables['XTIME'][t]))
    storm_xtimes = np.append(storm_xtimes, xtime)
    now = start + timedelta(minutes = int(xtime))
    subset=(land_type>1.8)
    index=np.argmin(slp[subset])
    lat_min=np.where(subset)[0][index]
    lon_min=np.where(subset)[1][index]
    slp_min = slp[lat_min, lon_min]
    slp_min=slp_min*.01
    lat_min_real = lats[lat_min, lon_min]
    lon_min_real = lons[lat_min, lon_min]
    #now the stuff we need for potential intensity:
    sst=np.copy(wrfout.variables['SST'][t, lat_min, lon_min])
    sst=sst-273.15
    mixing_ratio=np.copy(wrfout.variables['QVAPOR'][t, :, lat_min, lon_min])
    mixing_ratio=mixing_ratio*1000 
    pressure=getvar(wrfout,'pressure')
    pressure=np.array(pressure)
    point_pressure=pressure[:, lat_min, lon_min]
    p_level=np.copy(wrfout.variables['P'][t, :, lat_min, lon_min])
    # p_level=p_level*.01
    p_terb=np.copy(wrfout.variables['PB'][t, :, lat_min, lon_min])
    # p_terb=p_terb*.01
    P=p_level+p_terb
    theta=np.copy(wrfout.variables['T'][t, :, lat_min, lon_min])
    t_terb=wrfout.variables['T00'][t]
    temp=theta+t_terb 
    T=wrf.tk(point_pre, temp, meta=True, units='K')
    T=np.array(T)
    pres=P*.01
    p_terb=p_terb*.01
    pres=pres[T>100]
    mixing_ratio=mixing_ratio[T>100]
    T=T[T>100]
    T=T-273.15
    output=pi(sst, slp_min, point_pressure, T, mixing_ratio, CKCD=0.9, ascent_flag=0, diss_flag=1, V_reduc=0.8, miss_handle=1)
    print(output)
...
#%% holdover I didn't want to delete so someone could bear witness to my utter failure:
# names = ('ATCF','datetime', 'lat', 'lon', 'pres', 'speed')
# format = '%Y/%m/%d%H:%M:%S'
# df = pd.read_csv('pres_list_0', delim_whitespace=True, names=names)
# df=pd.DataFrame(df)
# df['datetime'] = pd.to_datetime(df['datetime'], format='%Y-%m-%d_%H:%M:%S')    
# total_rows=len(df.index)
# test=df.pivot_table(index='datetime')
# mpi_out=np.array([])
# for i in range(total_rows):
#     command='grep ATCF control' + str(i) + '/rsl.out.0000 >> pres_list_' + str(i) 
#     os.system(command)

# slp = wrfout.variables['PSFC'][0, mpi_lon ,mpi_lat]
# times= wrfout.variables['XTIME'][:]
# times = np.array(wrfout.variables['PSFC']).shape[0]
# lons = wrfout.variables['XLONG'][:, :, :]
# xlon=np.where(lons=mpi_lon)
# slp = slp*.01
# sst = wrfout.variables['SST'][index, mpi_lat ,mpi_lon]
