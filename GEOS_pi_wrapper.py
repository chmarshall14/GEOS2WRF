# -*- coding: utf-8 -*-
"""
Created on Mon Aug 17 13:49:47 2020
...
import netCDF4 
import xarray as xr
from namelist_geos_scripts import download_start, path_to_storm, download_end, start_lat, start_lon



#%%
#This script gets the necessary fields to calculate potential intensity 

#you only have to input the time and the location
#first get the time
...
#for now, just pick a point--soon we will figure out how to do this for a geographical region
lat=21
lon=-89

#%%
#get the location indeces:
def GEOS_Lon(n):
    return((n+180.0)/.0625 +1)
...
lat_inds=GEOS_Lat(lat)
lon_inds=GEOS_Lon(lon)
...
#it's in Pa, we need it in hPa
slp=slp*.01
print(slp)
#get sea-surface temperature
sst=np.copy(met1.variables['ts'][moment, lat_inds, lon_inds])
#its in Kelvin, we want it in Celsius
sst=sst-273.15
print(sst)

#we use specific humidity as a substitue for mixing ratio:
qv_file='https://opendap.nccs.nasa.gov/dods/OSSE/G5NR/Ganymed/7km/0.0625_deg/inst/inst30mn_3d_QV_Nv'
qv=Dataset(qv_file,'r')
qv=np.copy(qv.variables['qv'][moment,:,lat_inds,lon_inds])
qv=np.flip(qv)
qv=qv*1000 
print(qv)

#also get the pressure levels:
P_file='https://opendap.nccs.nasa.gov/dods/OSSE/G5NR/Ganymed/7km/0.0625_deg/inst/inst30mn_3d_PL_Nv'
P_lev=Dataset(P_file,'r')
P=np.copy(P_lev.variables['pl'][moment,:,lat_inds,lon_inds])
#convert Pa to hPa
P=P*.01
P=np.flip(P)
print(P)
#we also need the T profile:
T_file='https://opendap.nccs.nasa.gov/dods/OSSE/G5NR/Ganymed/7km/0.0625_deg/inst/inst30mn_3d_T_Nv'
T=Dataset(T_file,'r')
T=np.copy(T.variables['t'][moment,:,lat_inds,lon_inds])
T=T-273.15
T=np.flip(T)
print(T)
...
#%% 
output=pi(sst, slp, P, T, qv, CKCD=0.9, ascent_flag=0, diss_flag=1, V_reduc=0.8, miss_handle=1)
print(output)